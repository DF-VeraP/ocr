...
export const ReportView: React.FC<ReportViewProps> = ({ batchId, report }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 sm:p-8 shadow-sm border border-slate-200 dark:border-slate-700">
      ...
      {/* Vista TRANSFORMADA para Dispositivos Móviles (Tarjetas individuales táctiles) */}
      <div className="block md:hidden space-y-3.5">
        {report.map((item, idx) => {
          ...
          return (
            <div
              key={idx}
              className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-700/80 shadow-sm space-y-3"
            >
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold border ${tipoBadge.cls}`}>
                      {tipoBadge.label}
                    </span>
                ...
                {item.estadoValidacion === 'EXISTE' ? (
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-emerald-100 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-400 flex-shrink-0 flex items-center gap-1">
                    <CheckCircle className="w-3 h-3" /> Coincide
                  </span>
                ) : item.estadoValidacion === 'DISCREPANCIA' ? (
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-amber-100 dark:bg-amber-950/60 text-amber-700 dark:text-amber-400 flex-shrink-0 flex items-center gap-1">
                    <AlertCircle className="w-3 h-3" /> Discrepancia
                  </span>
                ) : (
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-red-100 dark:bg-red-950/60 text-red-700 dark:text-red-400 flex-shrink-0 flex items-center gap-1">
                    <XCircle className="w-3 h-3" /> Faltante
                  </span>
                )}
              </div>

              <div className="grid grid-cols-2 gap-2 text-xs pt-2 border-t border-slate-200/60 dark:border-slate-800/80">
                <div>
                  <span className="text-[10px] uppercase font-bold text-slate-400 block">Lectura OCR:</span>
                  <span className="text-slate-700 dark:text-slate-300 font-medium">
                    {item.nombreOcr || <em className="text-slate-400">No detectado</em>}
                  </span>
                </div>
                <div>
                  <span className="text-[10px] uppercase font-bold text-slate-400 block">Nacimiento (OCR):</span>
                  <span className="font-mono text-slate-700 dark:text-slate-300 font-medium">
                    {item.fechaNacimiento || <em className="text-slate-400">N/A</em>}
                  </span>
                </div>
              </div>

              {item.discrepancias && item.discrepancias.length > 0 && (
                <div className="p-2.5 rounded-xl bg-amber-50/80 dark:bg-amber-950/30 border border-amber-200/80 dark:border-amber-900/40 text-xs text-amber-800 dark:text-amber-300">
                  <span className="font-bold block mb-0.5">Discrepancia detectada:</span>
                  <span>{item.discrepancias[0].campo}: OCR leyó &quot;{item.discrepancias[0].valorOcr}&quot;</span>
                </div>
              )}
            </div>
          );
      ...
      {/* Tabla Oficial Completa para Pantallas Medianas / Grandes (hidden en mobile) */}
      <div className="hidden md:block overflow-x-auto overflow-y-auto max-h-[500px] print:max-h-none print:overflow-visible rounded-xl border border-slate-200 dark:border-slate-700 shadow-inner relative">
